...
export default function ProfileTab({ profile, setProfile }) {
  ...
  const [creating, setCreating] = useState(false); // ✅ for create modal
  ...
  const createProfile = async () => {
    try {
      console.log("📤 Creating profile with:", form); // debug
  ...
  const saveEdit = async () => {
    try {
      ...
      setProfile(res.data);  // ✅ refresh state with updated profile
      setEditing(false);
    } catch (err) {
